...
applyRollsUntilEnd :: Int -> GameState -> [Int] -> GameState
applyRollsUntilEnd _ gs [] = gs
applyRollsUntilEnd threshold gs rs = let
  (nextRolls, remainder) = splitAt 3 rs
  nextState = applyRollSet gs (sum nextRolls)
  in case (hasWinner threshold gs) of
    True -> gs
    False -> applyRollsUntilEnd threshold nextState remainder
...
part1Answer start1 start2 = let
  ...
  finalState = applyRollsUntilEnd 1000 initialState infiniteRolls
...
type UniverseCount = Map GameState Int
