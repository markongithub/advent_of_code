...
advanceGame state = let
  in if (null $ startingNumbersAcc state) 
  
(GameState (x:xs) lastSpoken currentTurn memory) = let
  currentNumber case starting of

speakStartingNumber :: GameState -> (Int, GameState)
speakStartingNumber (GameState (x:xs) _ currentTurn memory) = let
  nextSpoken = Map.findWithDefault 0 x memory
  newMemory = Map.insert x currentTurn memory
  in (x, GameState xs nextSpoken (currentTurn + 1) newMemory)

followLastNumber :: GameState -> GameState
followLastNumber (GameState _ lastNumber currentTurn memory) = let
  nextSpoken = Map.findWithDefault 0 lastNumber
  
  
  GameState xs x (currentTurn + 1) (Map.insert x currentTurn memory)
